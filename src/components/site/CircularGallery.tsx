...
class App {
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    // If swiping horizontally, prevent browser vertical page scrolling
    if (dx > dy && dx > 8) {
      if (e.cancelable) {
        e.preventDefault();
      }
  ...
  nudge() {
    if (this.hasInteracted || !this.medias || !this.medias[0]) return;
    const width = this.medias[0].width;
    // Nudge target forward to show there are photos on the right
    this.scroll.target = width * 0.35;
    setTimeout(() => {
      if (this.hasInteracted) return;
